...
const _cleanUrl = (url) => {
    ...
    if (isURLClean)
        return _cleanUrl(url.substring(0, url.length - 1));
    return url;
};
...
const _getPostData = async (postId, i = 0) => {
    try {
        ...
        if (i < 2)
            return await _getPostData(postId, i++);
        return null;
    }
};
...
const getAllMedia = async (url) => {
    try {
        ...
        if (!postData)
            return { msg: "invalid url or blocked." };
        const containedThreads = postData.containing_thread.thread_items;
        if (!containedThreads[0]?.post?.user)
        ...
        if (allMedia[0].media.length === undefined) {
            allMedia = allMedia.filter((m) => m.media?.url);
        }
        else {
